...
public class HandleAllgemeinItem : MonoBehaviour
{
	...
	public void HandleOnItemClick (InventoryItemDisplay itemDisplay)
	{
		...
		GameObject fachAllgemein = GameObject.Find ("GewähltAllgemein");
		...
		if (itemDisplay.item.activated) {
			...
			bool addMore = ((lpHelper.LernPunkteAllgemeinWissen - lernpunkteDelta) >= 0) ? true : false;
			if (addMore) {
				...
				lpHelper.lernPunkteResetAllgemein = false; //jetzt kann das Panel mit Inhalt bleiben
			}
		}
	}
	...
	{
		lpHelper.LernPunkteAllgemeinWissen -= deltaPoints;
		inputPointsLeft.text = lpHelper.LernPunkteAllgemeinWissen.ToString ();
